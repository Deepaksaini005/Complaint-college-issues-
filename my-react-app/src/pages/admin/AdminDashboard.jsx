...
const initialComplaintsData = {
  ...
  byDepartment: [],
  byStatus: [
    { name: 'Active', value: 0, color: '#ffc658' },
    { name: 'Resolved', value: 0, color: '#82ca9d' },
    { name: 'Closed', value: 0, color: '#8884d8' }
  ],
  trends: []
}
...
const AdminDashboard = ({ onLogout, userRole = 'Super Admin' }) => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        setLoading(true)
        const stats = await complaintAPI.stats()
        const inactive = Math.max(stats.total - stats.active - stats.resolved, 0)
        const departmentPalette = ['#CD201F', '#B34D3A', '#E94B4A', '#10b981', '#f59e0b', '#6366f1']
        const mappedDepartments = stats.departments.map((dept, index) => ({
        ...
        setComplaintsData({
          total: stats.total,
          byDepartment: mappedDepartments,
          byStatus: [
            { name: 'Active', value: stats.active, color: '#ffc658' },
            { name: 'Resolved', value: stats.resolved, color: '#82ca9d' },
            { name: 'Closed', value: inactive, color: '#8884d8' }
          ],
          trends: []
        })
        setError('')
  ...
  const statusPercentage = {
    active:
      complaintsData.total > 0
        ? (((complaintsData.byStatus[0]?.value || 0) / complaintsData.total) * 100).toFixed(1)
        : 0,
    resolved:
      complaintsData.total > 0
        ? (((complaintsData.byStatus[1]?.value || 0) / complaintsData.total) * 100).toFixed(1)
        : 0,
    closed:
      complaintsData.total > 0
        ? (((complaintsData.byStatus[2]?.value || 0) / complaintsData.total) * 100).toFixed(1)
        : 0
  }

  const COLORS = ['#ffc658', '#82ca9d', '#8884d8', '#f97316', '#10b981']
      ...
      <div className="dashboard-wrapper">
        ...
        <div className="summary-cards">
          ...
          <div className="summary-card active">
            ...
            <div className="card-content">
              <h3>Active</h3>
              <p className="card-value">{complaintsData.byStatus[0]?.value || 0}</p>
              <span className="card-label">{statusPercentage.active}% of total</span>
            </div>
          </div>
          ...
          <div className="summary-card resolved">
            ...
            <div className="card-content">
              <h3>Resolved</h3>
              <p className="card-value">{complaintsData.byStatus[1]?.value || 0}</p>
          ...
          <div className="summary-card closed">
            ...
            <div className="card-content">
              <h3>Closed</h3>
              <p className="card-value">{complaintsData.byStatus[2]?.value || 0}</p>
              <span className="card-label">{statusPercentage.closed}% of total</span>
            </div>
          </div>
        </div>
