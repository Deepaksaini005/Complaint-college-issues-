...
const HostelAdminLogin = ({ onLogin }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (data.user.role !== 'hostel') {
        setStatus({ state: 'error', message: 'This portal is restricted to hostel admins.' })
        return
      }
      ...
      navigate('/admin/hostel/dashboard')
    } catch (error) {
      setStatus({
        state: 'error',
  ...
  return (
    <div className="admin-portal">
      <section className="admin-card">
        <p className="eyebrow">Hostel Admin Portal</p>
        <h2>Hostel Admin Login</h2>
        <p className="intro">
          Login to manage hostel accommodations and resident services.
        </p>
        ...
        <form className="admin-form" onSubmit={handleSubmit}>
          <label>
            ...
            <input
              type="email"
              name="email"
              placeholder="hostel.admin@campus.edu"
          ...
          <button
            ...
            onClick={() => {
              const demoData = {
                email: 'hostel.admin@campuscare.com',
                password: 'admin123'
              }
