...
const AdminManagement = ({ onLogout, userRole = 'admin' }) => {
  ...
  const fetchAdmins = async () => {
    try {
      ...
      const mappedAdmins = data.map(admin => ({
        ...
        name: admin.name,
        email: admin.email,
        role: admin.displayRole || admin.role,
        department: admin.department,
        status: 'Active',
        createdAt: admin.createdAt ? new Date(admin.createdAt).toISOString().split('T')[0] : new Date().toISOString().split('T')[0]
      }))
